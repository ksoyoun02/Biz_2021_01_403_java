...
public class RndServiceImplV1_00 {

	// maktRnd와 saveFileRnd() method가 공용으로
	// 사용할 객체이므로 인스턴스 객체 변수로 선언
	private List<Integer> intList;
	
	// 생성자에도 매개변수를 붙일 수 있다.
	public RndServiceImplV1_00() {
	 ...
	 * 객체지향의 (메서드)다형성
	 * 
	 * 한 클래스 파일 내에서는 같은 이름의 method를 두번이상 정의할 수 없다.
	 * 
	 * 하지만 객체지향 언어에서는
	 * method의 매개변수가 있거나 없거나
	 * 또는 매개변수의 개수가 다르거나
	 * 매개변수의 type이 다르면
	 * 같은 이름의method를 중복정의(중복하여 정의)할수있다
	 * 
	...
	/*
	 * RndServiceImplV1 클래스를 처음 디자일 할때는
	 ...
	 * 코드에 포함된 파일이름을 적용하여
	 * saveFileRnd(파일 이름) method를 호출하도록
	 * 만들었다
	 * 
	 ...
	 * Random 수 들이 저장되어 있는 파일에서
	 * 
	 * Random 수 들을 다시 읽어들여
	 * 연산을 수행하는 method
	 * 
	 * 필요한 조건 -> 저장할 때 사용한 파일이름 == 읽을때 사용할 파일 이름 
	 * 
	 * 이 상황은 2개의 method가 1개의 공통된 파일 이름을 사용한다는 것이다.
	 * 
	 * 결국 savefileRnd() method만 사용하기 위해 선언된
	 * fileName 변수는 인스턴스 영역에 선언되어야 코드가 간편해질것이다.
	 */
