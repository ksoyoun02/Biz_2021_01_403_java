...
public class CartServiceImplV1 implements CartService {
	 ...
	 * 
	 * 이러한 경우에
	 * private으로 선언된 변수()객체는 서로 공유가 되지 않는다.
	 * 상속받은 클래스의 method에서 변수(객체를)
	 * 공유하려면 부모크래스에서
	 * 변수(객체)를 반드시 protected로 선언해 주어야 한다.
	 * 또한 상속받은 클래스 에서는 해당 변수(객체)를 선언해서는 안된다.
	 *  
	 * 
	 */
	// "List를 공유하겠다"
	...
	public void printAllCart() {
		...
		for (int i = 0; i < nSize; i++) {
			...
			/*
			 * cartList.get(i)는 cartList에 저장되어있는 i번째 요소,
			...
			// cartVO 객체에 임시(for() 명령 내에 있기 때문에)선언된
