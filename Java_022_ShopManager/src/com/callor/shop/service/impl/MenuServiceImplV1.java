...
public class MenuServiceImplV1 implements MenuService {
	...
	/*
	 * final 키워드는 변수, 객체를 혹시모를 어디선가 변경하는 행위를 못하도록
	 *  방지하는 조치, 상수언이라고 한다
	 *  
	 * final로 선언된 변수는 객체는 읽기 전용이다
	 * 
	 * 값을 저장하려고 시도하면 오류가 난다.
	 * 
	 * final int num = 0; num = 100; <- 요류가 발생한다.
	 */
	...
	public Integer selectMenu() {
		...
		while(true) {
			...
			*/
			...
			// try문이랑 if문을 분리해 주는 것이 가장 최적화된 코드
			if(intMenu >= Values.MENU_START && intMenu <= Values.MENU_LAST) {
				return intMenu;
			}else {
				System.out.printf("업무는 %d ~ %d 까지 중에서 선택", Values.MENU_START, Values.MENU_LAST);
			}
