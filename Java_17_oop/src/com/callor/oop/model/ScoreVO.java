 ...
 * setIntKor() method는 intKor 인스턴수변수에 값을 저장
 * getIntKor() method는 itnKor 인스턴수변수에서 값을 읽기 기능을 한다는 것을 알 수있다.
 * 
 * 코드 내용이 어떠한지 몰라도
 * getter와 setter를 통하여 변수에 접근(저장, 읽기)할수있다.
 * 
 */
...
public class ScoreVO {
	...
	/*
	 * 국어점수(intKor)를 저장할 인스턴스 변수를 private로 선언하여 직접 접근할수 없도록 만든다
	 * 
	 * 국어점수(intKor) 인스턴스 변수에 간접적으로 값을 저장하는 setIntKor() method를 선언
	 * 
	 * setIntKor() method는 점수를 문자열로 받아서 정수로 변경한 다음 범위검사를 수행하고 
	 * 유효성통과(정상적인 점수일경우)를 한 경우만 국어 인스턴스변수(this.intKor)에 저장한다.
	 * 
	 ...
	 * intEng 변수에 저장(setting)할수있도록 만드는것
	 */
	 ...
	 * intEng 변수의 값을 읽기위해서 getIntEng()라는 메서드를
	 * 또 하나 선언해 둔다.
	 */
	public int gitIntEng() {
		return this.intEng;
	}
